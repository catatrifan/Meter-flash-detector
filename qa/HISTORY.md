...
## 2026-09-23 — Fresh independent QA review of current main
- Re-inspected the current main branch directly without treating prior QA conclusions as authoritative.
- Re-verified that duplicate Pulse constant IDs, per-frame canvas allocation, pause-duration contamination of the exact average, invalid-interval counting, the non-async camera-start issue, and literal escaped-newline regression are not present in the current source.
- Added QA-030 (HIGH): the visible “Pulses detected” statistic uses the complete-interval count, so it under-reports accepted flashes by one.
- Added QA-031 (HIGH): invalid intervals longer than the supported range still replace lastPulseTime and can therefore redefine the timing anchor for the next interval.
- Added QA-032 (MEDIUM): duplicate helper definitions increase maintenance/regression risk.
- Added QA-033 (MEDIUM): provisional average-assessment timing has separate semantics from the exact active-time average and changes timing window at the first pulse.
